...
export default function StatusLabelEdge({
  id,
  sourceX,
  sourceY,
  targetX,
      ...
      <EdgeLabelRenderer>
        ...
        >
          {Object.keys(data).map((c) => (
            <div key={c} className="flex flex-col p-1">
              <div className="text-[7px] text-gray-500 uppercase text-center">{c}</div>
              {statusIcons[data[c].status]}
              {null !== data[c].error && (
                <div
                  title={data[c].error?.errorMessage}
                  className="text-[7px] text-red-700 border-dotted border-b-1 cursor-pointer"
                >
                  {data[c].error?.errorNumber}
                </div>
              )}
            </div>
          ))}
