...
export default function TableOperations({ stats }: TableOperationsProps) {
    ...
    const operations = useMemo<string[]>(() => {
        ...
        return Array.from(
            new Set(
                Array.from(stats.values()).flatMap((tableStats) =>
                    Array.from(tableStats.keys())
                )
            )
        );
    }, [stats]);
    ...
    const sortedTables = useMemo<[string, OperationStats][]>(() => {
        ...
        return Array.from(stats.entries()).sort(
            ([, aStats], [, bStats]) => {
                const sumA = Array.from(aStats.values()).reduce(
                    (acc, v) => acc + v,
                    0
                );
                const sumB = Array.from(bStats.values()).reduce(
                    (acc, v) => acc + v,
                    0
                );
                return sumB - sumA;
    ...
    useEffect(() => {
        ...
        for (const [table, tableStats] of stats.entries()) {
            const total = Array.from(tableStats.values()).reduce(
                (acc, v) => acc + v,
                0
            );
                    ...
                    return (
                        ...
                        >
                            ...
                            {operations.map((op) => (
                                <td
                                    key={op}
                                    className="px-4 py-2 text-sm text-right text-gray-700"
                                >
